...
Open-source BYOK web translation for careful bilingual reading.

## Detailed Description

OpenRead is an open-source browser extension for people who want more control over web translation.

It translates web pages in place, supports quick right-click selection translation, and lets you configure your own OpenAI-compatible provider instead of relying on a bundled subscription plan. It is designed for careful reading: articles, documentation, research pages, technical posts, and other long-form web content where preserving context matters.
...
OpenRead is still early software. Some complex pages may render imperfectly, and site-specific optimizations are still being improved. The project is intended to stay open, inspectable, and hackable.

## 中文简介

OpenRead 是一个开源、BYOK 的网页翻译浏览器扩展，面向需要认真阅读外文网页、技术文档、文章和资料的用户。
...
OpenRead 目前仍是早期版本，复杂页面可能存在渲染不完美或漏翻问题。项目目标是保持开放、可检查、可修改。

## Permission Rationale

- `storage`: saves provider profiles, prompts, language settings, display settings, and local translation cache.
...
- `<all_urls>` host permission: allows page translation and selection translation across ordinary webpages.
